...
menu_local_ai() {
    if [ "$LOCAL_AI_STATUS" = "Running" ]; then
        ...
        sleep 3
        # Auto-configure config.yaml with local provider
        if ! grep -q "###LOCAL_AI_CONFIG###" "$HERMES_HOME/config.yaml" 2>/dev/null; then
            cat >> "$HERMES_HOME/config.yaml" << EOF

# ###LOCAL_AI_CONFIG### - auto-generated by launch.sh
model:
  default: deepseek-v4-flash-free
  provider: custom:local-ai
providers:
  local-ai:
    name: Local AI (opencode-openai - free)
    base_url: http://127.0.0.1:8787/v1
    api_mode: chat_completions
    discover_models: true
EOF
            echo -e "${BRIGHT_GREEN}  Config auto-set to http://127.0.0.1:8787/v1${RESET}"
        else
