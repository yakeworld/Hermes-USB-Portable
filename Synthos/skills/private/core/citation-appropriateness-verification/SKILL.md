...
## Pitfalls
- **bib DOI 可能是错误的**: 已在 HCS-3WT 论文中确认，Chakravarthy2021Deep 的 DOI 指向完全不同的植物生物学论文。引用实质审查前必须先 Crossref 验证每条 DOI。详见 `references/bib-doi-validation-2026-07-06.md`。
- **PDF 文件名与 bib key 不匹配**: PDF 可能以不同命名存储（如 `Sung2021GlobalBC` vs `Sung2021Global`），需部分匹配而非精确匹配。
- **PDF 可能存放在非标准目录**: 检查 `06-references/pdfs/`、`06-references/pdfs/new/`、`academic_writer/*/enhanced_bibtex/pdfs/` 等多个位置。
- **Sci-Hub 2022+ Springer/Elsevier 覆盖不足**: 2022 年后的 Springer 和 Elsevier 论文在 Sci-Hub 上可能无 PDF，需优先尝试 OA 来源。
- **JAMA 等期刊直接 curl 返回 403**: 需通过 PMC/Europe PMC 间接获取，或直接使用 Crossref OA link。 
...
### 第五层（新增）：DOI 真实性验证（⚠️ 2026-07-06 新增）

在全文阅读前，必须验证每条引用的 DOI 是否指向 bib 条目描述的文献。**bib 中的 DOI 可能是错误的**，已在 HCS-3WT 论文中确认（Chakravarthy2021Deep 的 DOI 指向了完全不同的植物生物学论文）。

验证步骤：
1. 对每条有 DOI 的引用，用 Crossref API 查询 DOI → 标题/作者/期刊/年份
2. 将查询结果与 bib 条目中的 title/author/year 逐字比对
3. 标题或作者不匹配 → 标记为 `DOI_MISMATCH`，不能使用该 DOI 下载 PDF
4. 无 DOI 的引用（书籍、数据集、政策文件等）跳过此步

参考：`references/bib-doi-validation-2026-07-06.md`

### 第六层：引用功能分类与引用统计

（原有的第一层引用功能分类移至此处，作为完整审查的最终输出）
